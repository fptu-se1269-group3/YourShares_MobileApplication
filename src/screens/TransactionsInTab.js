...
export default class TransactionsInTab extends Component {
    constructor(props) {
        super(props);

        this.state = {
            transactions: this.props.arrayTransaction.filter(tr => tr.transactionTypeCode === 'IN')
        }
    }
        ...
        if (oldProps.selected !== newProps.selected || oldProps.date !== newProps.date || oldProps.date2 !== newProps.date2) {
            ...
            if (this.props.selected === 'all') {
                ...
                queryObj = jslinq(myList)
                    ...
                    .where(item => item.transactionTypeCode === 'IN')
